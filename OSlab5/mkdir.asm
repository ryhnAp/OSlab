...
main(int argc, char *argv[])
{
  ...
  exit();
    ...
    1058:	6a 02                	push   $0x2
    105a:	e8 01 04 00 00       	call   1460 <printf>
      break;
    ...
    106b:	6a 02                	push   $0x2
    106d:	e8 ee 03 00 00       	call   1460 <printf>
    exit();
...
printf(int fd, const char *fmt, ...)
{
  ...
  write(fd, &c, 1);
    ...
    15e4:	eb 1a                	jmp    1600 <printf+0x1a0>
          s = "(null)";
    15e6:	bb fb 17 00 00       	mov    $0x17fb,%ebx
        while(*s != 0){
...
free(void *ap)
{
  ...
  for(p = freep; !(bp > p && bp < p->s.ptr); p = p->s.ptr)
    1635:	a1 bc 1a 00 00       	mov    0x1abc,%eax
{
    ...
    1656:	73 10                	jae    1668 <free+0x38>
{
  ...
  freep = p;
    ...
    1681:	5b                   	pop    %ebx
  freep = p;
    1682:	a3 bc 1a 00 00       	mov    %eax,0x1abc
}
    1687:	5e                   	pop    %esi
    1688:	5f                   	pop    %edi
    1689:	5d                   	pop    %ebp
  ...
  freep = p;
    16aa:	a3 bc 1a 00 00       	mov    %eax,0x1abc
    p->s.size += bp->s.size;
...
malloc(uint nbytes)
{
  ...
  if((prevp = freep) == 0){
    16d0:	8b 3d bc 1a 00 00    	mov    0x1abc,%edi
  nunits = (nbytes + sizeof(Header) - 1)/sizeof(Header) + 1;
  ...
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
    ...
    1719:	8b 3d bc 1a 00 00    	mov    0x1abc,%edi
  ...
  return freep;
    1747:	a1 bc 1a 00 00       	mov    0x1abc,%eax
    ...
    1774:	89 72 04             	mov    %esi,0x4(%edx)
      freep = prevp;
    1777:	a3 bc 1a 00 00       	mov    %eax,0x1abc
}
    ...
    base.s.ptr = freep = prevp = &base;
    1790:	c7 05 bc 1a 00 00 c0 	movl   $0x1ac0,0x1abc
    1797:	1a 00 00 
    base.s.size = 0;
    179a:	bf c0 1a 00 00       	mov    $0x1ac0,%edi
    base.s.ptr = freep = prevp = &base;
    179f:	c7 05 c0 1a 00 00 c0 	movl   $0x1ac0,0x1ac0
    17a6:	1a 00 00 
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
    17a9:	89 f8                	mov    %edi,%eax
    base.s.size = 0;
    17ab:	c7 05 c4 1a 00 00 00 	movl   $0x0,0x1ac4
    17b2:	00 00 00 
    if(p->s.size >= nunits){
